    ...
    <header className="sticky top-0 z-50 bg-card/95 backdrop-blur-md border-b border-border/50 shadow-brand">
      <div className="container mx-auto flex items-center justify-between h-20 lg:h-24 px-4">
        ...
        <nav className="hidden lg:flex items-center gap-1">
          <Link to="/" className={`px-4 py-2 rounded-lg text-sm font-semibold transition-colors duration-300 ${isActive("/") ? "text-secondary" : "text-primary hover:text-secondary"}`}>
            Home
          </Link>

          {/* Personal Dropdown */}
          <div className="relative" onMouseEnter={() => setActiveDropdown("personal")} onMouseLeave={() => setActiveDropdown(null)}>
            <button className={`flex items-center gap-1 px-4 py-2 rounded-lg text-sm font-semibold transition-colors duration-300 ${activeDropdown === "personal" ? "text-secondary" : "text-primary hover:text-secondary"}`}>
            ...
            <AnimatePresence>
              {activeDropdown === "personal" && (
                <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 8 }} transition={{ duration: 0.15 }}
                  ...
                  {personalLinks.map(l => (
                    <Link key={l.href} to={l.href} className="block px-4 py-2.5 text-sm text-foreground hover:text-secondary hover:bg-muted transition-colors duration-200">{l.label}</Link>
                  ))}
                </motion.div>
              )}
            </AnimatePresence>
          ...
          <div className="relative" onMouseEnter={() => setActiveDropdown("business")} onMouseLeave={() => setActiveDropdown(null)}>
            <button className={`flex items-center gap-1 px-4 py-2 rounded-lg text-sm font-semibold transition-colors duration-300 ${activeDropdown === "business" ? "text-secondary" : "text-primary hover:text-secondary"}`}>
          ...
          <Link to="/about" className={`px-4 py-2 rounded-lg text-sm font-semibold transition-colors duration-300 ${isActive("/about") ? "text-secondary" : "text-primary hover:text-secondary"}`}>
            About
          </Link>
        </nav>
        ...
        <div className="flex items-center gap-3">
          <a href="tel:+14254057111" className="hidden sm:flex items-center gap-2 text-sm font-semibold text-primary hover:text-secondary transition-colors duration-300">
        ...
        {mobileOpen && (
          <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="lg:hidden overflow-hidden bg-card border-t border-border/50">
            <div className="container mx-auto px-4 py-4 space-y-1">
              <Link to="/" onClick={() => setMobileOpen(false)} className="block px-4 py-3 rounded-lg text-sm font-semibold text-primary hover:bg-muted">Home</Link>
              ...
              <Link to="/about" onClick={() => setMobileOpen(false)} className="block px-4 py-3 rounded-lg text-sm font-semibold text-primary hover:bg-muted">About</Link>
